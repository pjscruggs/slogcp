...
- [Send logs through the Cloud Logging API](cloud-logging-grpc.md) when using
  the optional `slogcp-grpc` module with the official client's batching,
  buffering, concurrency, and error controls.
...
- [Keep existing Pub/Sub OpenTelemetry instrumentation](pubsub-existing-otel.md)
  when importing the optional `slogcp-pubsub` module to add message fields and
  completion logs while preserving the subscriber callback's existing span.
