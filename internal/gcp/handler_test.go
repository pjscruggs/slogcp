...
// checking how slog.Records are converted into logging.Entry structs with
// appropriate severity, trace context, source location, payload, and error formatting.
func TestGcpHandler_Handle(t *testing.T) {
	...
	testStackErr := newStackError("stack error msg", "myFunc()\n\t/path/to/file.go:10")
	// Correctly initialize logging.HTTPRequest by embedding an *http.Request.
	testHTTPRequestForLog, err := http.NewRequest("POST", "/test", nil)
	if err != nil {
		t.Fatalf("Failed to create test HTTP request: %v", err)
	}
	testHTTPEntry := &logging.HTTPRequest{Request: testHTTPRequestForLog, Status: 400}
	...
	type testCase struct {
		...
		wantPayloadCheck func(t *testing.T, payload map[string]any)               // Custom checks for payload details
	}
	...
	testCases := []testCase{
		...
		},
		{
			...
			record: func() slog.Record {
				...
				r.AddAttrs(slog.Any("err", testBasicErr))
				return r
			}(),
		...
		},
		{
			...
			record: func() slog.Record {
				...
				r := slog.NewRecord(baseTime, slog.LevelError, "Failure", getHandlerTestPC())
				r.AddAttrs(slog.Any("error", testBasicErr))
				return r
			}(),
			wantSeverity: logging.Error,
			wantPayload: map[string]any{ // Base payload check ignores error/stack keys
				messageKey:   "Failure",
				"error":      testBasicErr,
				errorTypeKey: "*errors.errorString",
			},
			wantPayloadCheck: func(t *testing.T, payload map[string]any) {
				...
				// The exact frame might be the anonymous func inside t.Run.
				...
				if strings.Contains(stackStr, "formatErrorForReporting") {
					t.Errorf("Fallback stack trace unexpectedly contains 'formatErrorForReporting':\n%s", stackStr)
				}
				if strings.Contains(stackStr, "testing.tRunner") {
					t.Errorf("Fallback stack trace unexpectedly contains 'testing.tRunner':\n%s", stackStr)
				}
				if strings.Contains(stackStr, "runtime.goexit") {
					t.Errorf("Fallback stack trace unexpectedly contains 'runtime.goexit':\n%s", stackStr)
				}
			},
		},
		{
			...
			record: func() slog.Record {
				...
				// testStackErr now implements stackTracer via common_test.go update
				r.AddAttrs(slog.Any("detail", testStackErr))
				return r
			}(),
			wantSeverity: logging.Error,
			wantPayload: map[string]any{
				...
				"detail":     testStackErr,
				errorTypeKey: "*gcp.stackError", // Type of the error found
			},
			wantPayloadCheck: func(t *testing.T, payload map[string]any) {
				stackVal, ok := payload[stackTraceKey]
				if !ok {
				...
				// which should include the caller within the test setup.
				// The specific caller is newStackError itself in this test setup.
		...
		},
		{
			...
			ctx:          func() context.Context { ctx, _, _, _ := contextWithTrace(projectID); return ctx }(), // Generate context with trace
			record:       slog.NewRecord(baseTime, slog.LevelInfo, "Tracing this", 0),                          // No attrs needed here
			wantSeverity: logging.Info,
		...
		},
		{
			...
			record:       slog.NewRecord(baseTime, slog.LevelInfo, "Source test", getHandlerTestPC()),
			wantSeverity: logging.Info,
			wantSourceCheck: func(t *testing.T, sl *loggingpb.LogEntrySourceLocation) {
				...
				if !strings.HasSuffix(sl.File, wantFileSuffix) {
					t.Errorf("SourceLocation.File: got %q, want suffix %q", sl.File, wantFileSuffix)
				}
				// Now expect the caller of getHandlerTestPC, which is the test function.
				wantFuncSubstring := "TestGcpHandler_Handle"
				if !strings.Contains(sl.Function, wantFuncSubstring) {
					t.Errorf("SourceLocation.Function: got %q, want substring %q", sl.Function, wantFuncSubstring)
				}
				if sl.Line <= 0 {
					t.Errorf("SourceLocation.Line expected positive, got %d", sl.Line)
				}
			},
			wantPayload: map[string]any{
		...
		},
		{
			...
			record:       slog.NewRecord(baseTime, slog.LevelInfo, "No source", getHandlerTestPC()), // PC is present but ignored
			wantSeverity: logging.Info,
			wantSourceCheck: func(t *testing.T, sl *loggingpb.LogEntrySourceLocation) {
				if sl != nil {
		...
		},
		{
			...
			wantPayloadCheck: func(t *testing.T, payload map[string]any) {
				...
				if payload[errorTypeKey] != "*errors.errorString" {
					t.Errorf("Expected errorType from first error (*errors.errorString), got %v", payload[errorTypeKey])
				}
				// Check both original errors are still present in the payload
				if _, ok := payload["error1"]; !ok {
					t.Error("Missing original attribute 'error1'")
				}
				if _, ok := payload["error2"]; !ok {
					t.Error("Missing original attribute 'error2'")
				}
				// Check stack trace exists (from first error's fallback)
				if _, ok := payload[stackTraceKey]; !ok {
					t.Error("Missing stackTrace key")
				} else {
					// Verify fallback stack contains test function
					stackStr := payload[stackTraceKey].(string)
					if !strings.Contains(stackStr, "TestGcpHandler_Handle") {
						t.Errorf("Expected fallback stack trace containing 'TestGcpHandler_Handle', got:\n%s", stackStr)
					}
				}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.cfgOverride != nil {
				currentCfg.AddSource = tc.cfgOverride.AddSource
				currentCfg.StackTraceEnabled = tc.cfgOverride.StackTraceEnabled
				...
				if tc.cfgOverride.StackTraceLevel != 0 || tc.cfgOverride.StackTraceEnabled {
					currentCfg.StackTraceLevel = tc.cfgOverride.StackTraceLevel
				} else {
					currentCfg.StackTraceLevel = baseCfg.StackTraceLevel // Reset to base
				}
			}
			// Ensure handler uses a level that allows all test records through for verification.
			handler := NewGcpHandler(currentCfg, mockLogger, slog.LevelDebug)

			// Apply handler groups and attrs state for the test case.
			var finalHandler slog.Handler = handler // Use interface type
			// Apply groups first, then attributes, simulating typical usage pattern.
			if len(tc.handlerGroups) > 0 {
				var tempHandler slog.Handler = handler
				for _, group := range tc.handlerGroups {
					tempHandler = tempHandler.WithGroup(group) // Returns slog.Handler
				}
				finalHandler = tempHandler
			}
			if len(tc.handlerAttrs) > 0 {
				finalHandler = finalHandler.WithAttrs(tc.handlerAttrs) // Returns slog.Handler
			}
			...
			if !logged {
				t.Fatal("No log entry was recorded by the mock logger")
			}
			...
			// Ignore unexported fields in http.Request for comparison.
			httpOpts := []cmp.Option{cmpopts.IgnoreUnexported(*testHTTPRequestForLog)}
			...
			if !ok {
				t.Fatalf("Entry.Payload is not map[string]any, got %T", entry.Payload)
			}
			...
			if tc.wantPayload != nil {
				...
				// as these are handled by wantPayloadCheck or verified implicitly.
				ignoreKeys := []string{errorTypeKey, stackTraceKey}
				ignoreErrorAttr := cmpopts.IgnoreMapEntries(func(k string, v any) bool {
					for _, ignoreKey := range ignoreKeys {
						if k == ignoreKey {
							return true
						}
					}
					// Also ignore the original error attribute itself during general map compare.
					_, isErr := v.(error)
					return isErr
				})
				// EquateEmpty treats nil and empty maps/slices as equal.
				opts := []cmp.Option{ignoreErrorAttr, cmpopts.EquateEmpty()}
				if diff := cmp.Diff(tc.wantPayload, payloadMap, opts...); diff != "" {
				...
				// Exception: Allow errorTypeKey if an error was logged but no specific check was defined.
				if _, hasErrType := payloadMap[errorTypeKey]; !hasErrType || len(payloadMap) > 1 {
					t.Errorf("Expected empty payload (or only errorType), got %v", payloadMap)
				}
			}
		})
	}
...
func TestGcpHandler_WithGroup(t *testing.T) {
	...
	record.AddAttrs(slog.String("a", "b")) // Add attribute after creating record
...
func TestGcpHandler_WithAttrs(t *testing.T) {
	...
	// Log with the final handler (h2)
	record := slog.NewRecord(time.Now(), slog.LevelInfo, "msg", 0)
	record.AddAttrs(slog.String("d", "4")) // Add attribute after creating record
	err := h2.Handle(context.Background(), record)
	if err != nil {
		t.Fatalf("h2.Handle failed: %v", err)
	}
...
func TestGcpHandler_WithGroupAndAttrsInteraction(t *testing.T) {
	mockLogger := &mockEntryLogger{}
	baseCfg := Config{ProjectID: "proj"}
	// Use interface type for handler variables to reflect actual usage.
	...
	t.Run("Attrs_Group_Attrs", func(t *testing.T) {
		mockLogger.Reset()
		// Assign results back to slog.Handler interface variable.
		...
		record_ag.AddAttrs(slog.String("c", "3")) // Record attr 'c'
		...
		payload_ag := entry_ag.Payload.(map[string]any)
	...
	t.Run("Group_Attrs_Group_Attrs", func(t *testing.T) {
		...
		record_ga.AddAttrs(slog.String("c", "3")) // Record attr 'c'
		...
		payload_ga := entry_ga.Payload.(map[string]any)
